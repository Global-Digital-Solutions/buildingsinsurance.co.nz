...
export default function QuoteForm({ compact = false }: QuoteFormProps) {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', property_type: '', property_value: '', details: '',
  });
  ...
  const propertyTypeField = (
    <div>
      <label htmlFor="property-type" className={labelClass}>Property Type <span className="text-red-500">*</span></label>
      <select id="property-type" name="property_type" required value={formData.property_type} onChange={handleChange} className={`${inputClass} appearance-none`}>
  ...
  const propertyValueField = (
    <div>
      <label htmlFor="property-value" className={labelClass}>Estimated Value <span className="text-red-500">*</span></label>
      <select id="property-value" name="property_value" required value={formData.property_value} onChange={handleChange} className={`${inputClass} appearance-none`}>
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="text" id="name" name="name" required placeholder="John Smith" value={formData.name} onChange={handleChange} className={inputClass} />
        </div>
        <div>
          <label htmlFor="email" className={labelClass}>Email <span className="text-red-500">*</span></label>
          <input type="email" id="email" name="email" required placeholder="john@example.com" value={formData.email} onChange={handleChange} className={inputClass} />
        </div>
        <div>
        ...
        <p className="text-center text-slate-500 text-xs mt-3">Your data is secure and encrypted. We never share your information.</p>
      </form>
    );
  }
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" id="name" name="name" required placeholder="John Smith" value={formData.name} onChange={handleChange} className={`${inputClass} pl-12`} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" id="email" name="email" required placeholder="john@example.com" value={formData.email} onChange={handleChange} className={`${inputClass} pl-12`} />
              </div>
            </div>
            <div>
              <label htmlFor="phone" className={labelClass}>Phone <span className="text-red-500">*</span></label>
          ...
          <div>
            <label htmlFor="details" className={labelClass}>Additional Details</label>
            <textarea id="details" name="details" placeholder="Tell us more about your property (optional)..." rows={4} value={formData.details} onChange={handleChange} className={`${inputClass} resize-none`} />
          </div>
        </div>
